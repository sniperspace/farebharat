...
Reads directly from data/apix.db which is refreshed daily by the
...
st.caption(
    "APIx metric · Base = 100 · Jevons index · DGCA passenger-traffic weighted "
    "basket · aligned with CPI methodology · data refreshed daily via GitHub Actions"
)
...
with st.sidebar:
    ...
    st.write(f"**Total quotes:** {stats['quotes']:,}")
    if stats["latest_scrape"]:
        st.write(f"**Latest scrape:** {stats['latest_scrape']}")
    if stats["latest_index_date"]:
        st.write(f"**Latest index date:** {stats['latest_index_date']}")

    st.divider()
    st.markdown(
        "**Data pipeline**\n\n"
        "1. GitHub Actions cron (06:15 IST daily)\n"
        "2. Scrapers → raw JSON audit trail\n"
        "3. Cleaner → validated DB rows\n"
        "4. Index engine → APIx (Jevons)\n"
        "5. This dashboard auto-refreshes every 10 min"
    )
...
kpi2.metric("Quotes collected", f"{stats['quotes']:,}")
kpi3.metric(
    "Latest scrape",
    str(stats["latest_scrape"]) if stats["latest_scrape"] else "—",
)
kpi4.metric("Data source", "GitHub Actions (daily)")
...
with tab1:
    ...
    if df.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            name="APIx", line=dict(color="#1a3c6e", width=3),
        ))
...
with tab3:
    ...
    if quotes_df.empty:
        ...
        fig.update_layout(
            ...
            yaxis_title="Avg total fare (INR)",
            height=430,
        )
        st.plotly_chart(fig, use_container_width=True)
...
with tab4:
    ...
    if quotes_df.empty:
        ...
        fig.update_layout(height=430, yaxis_title="Avg fare (INR)")
        st.plotly_chart(fig, use_container_width=True)
...
st.caption(
    ...
    "Open source (MIT). Data is a live pipeline; scraper honours robots.txt "
